...
let evt = require(__dirname + "/bmbtech/popkid");
const {isUserBanned , addUserToBanList , removeUserFromBanList} = require("./lib/banUser");
const  {addGroupToBanList,isGroupBanned,removeGroupFromBanList} = require("./lib/banGroup");
const {isGroupOnlyAdmin,addGroupToOnlyAdminList,removeGroupFromOnlyAdminList} = require("./lib/onlyAdmin");
//const //{loadCmd}=require("/bmbtech/mesfonctions")
let { reagir } = require(__dirname + "/bmbtech/app");
var session = conf.session.replace(/bot-/g,"");
...
if (conf.AUTOREACT_STATUS=== "yes") {
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            var verifPopkidAdmin = verifGroupe ? admins.includes(idBot) : false;
            /** ** */
            /** ***** */
...
// Paste this block at the very end of main() function (before return zk;)
// Make sure axios is already required above, and conf.CHATBOT is on settings

const chatbotAPIs = [
    "https://api.giftedtech.co.ke/api/ai/openai?apikey=gifted&q=",
    "https://api.giftedtech.co.ke/api/ai/gpt4?apikey=gifted&q=",
    "https://api.giftedtech.co.ke/api/ai/gpt?apikey=gifted&q=",
    "https://api.giftedtech.co.ke/api/ai/chat?apikey=gifted&q=",
    "https://api.giftedtech.co.ke/api/ai/ai?apikey=gifted&q="
];
...
function randomAPI() {
    return chatbotAPIs[Math.floor(Math.random() * chatbotAPIs.length)];
}

// Name question detection
function getNameAnswer(txt) {
    txt = txt.toLowerCase().trim();
    if (
        txt.includes("unaitwa nani") ||
        txt.includes("jina lako nani") ||
        txt.includes("jina lako ni nani") ||
        txt.includes("wewe ni nani") ||
        txt.includes("una jina gani") ||
        txt.includes("unaitwa nani wewe")
    ) return "Naitwa bmb tech";
    if (
        txt.includes("what is your name") ||
        txt.includes("what's your name") ||
        txt.includes("who are you") ||
        txt.includes("your name") ||
        txt.includes("may i know your name")
    ) return "My name is bmb tech";
    return null;
}

// YES/NO detection
function isYesNoQuestion(txt) {
    txt = txt.toLowerCase().trim();
...
zk.ev.on("messages.upsert", async (m) => {
    ...
    // Get text
    let texte = "";
    if (ms.message.conversation) texte = ms.message.conversation;
    else if (ms.message.extendedTextMessage?.text) texte = ms.message.extendedTextMessage.text;
    else return;

    // Ignore commands & self
    if (texte.startsWith(prefixe)) return;
    if (ms.key.fromMe) return;

    // Name auto reply
    let nameAnswer = getNameAnswer(texte);
    if (nameAnswer) {
    ...
    // Try API
    try {
        let url = randomAPI() + encodeURIComponent(texte);
        let { data } = await axios.get(url, { timeout: 10000 });

        let reply = data.response || data.reply || data.result || data.answer || data.choices?.[0]?.text || data.message || data;
        ...
        // Fallback: never fail, always reply something
        const fallbacks = [
            "Okay 👍",
            "Sawa nimekupata 😎",
            "Hmm interesting...",
        ...
        let randomFallback = fallbacks[Math.floor(Math.random() * fallbacks.length)];
...
║══════════════════════`;
 ...
 //---------------------------------------rang-count--------------------------------
  ...
  try {
    ...
    let msgText = `Link detected, your warn_count was upgraded ;
rest : ${rest}`;

    await zk.ajouterUtilisateurAvecWarnCount(auteurMessage);
      ...
      await zk.sendMessage(origineMessage, { delete: key });
        ...
        console.log('.... ' + er);
            ...
            if (verifCom) {
                ...
                const cd = evt.cm.find((popkid) => popkid.nomCom === (com));
                if (cd) {
                    try {
