...
async function authentification() {
    try {
        ...
        if (!fs.existsSync(__dirname + "/sessions/creds.json")) {
            ...
            await fs.writeFileSync(__dirname + "/sessions/creds.json", atob(session), "utf8");
            //console.log(session)
        }
...
setTimeout(() => {
    async function main() {
        ...
        const { state, saveCreds } = await (0, baileys_1.useMultiFileAuthState)(__dirname + "/sessions");
        const sockOptions = {
            version,
            logger: pino({ level: "silent" }),
            browser: ['bmbtech-popkid', "safari", "1.0.0"],
