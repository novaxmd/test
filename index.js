...
// Auto-react to status updates (improved & throttled)
        if (conf.AUTO_REACT_STATUS === "yes") {
            ...
            zk.ev.on("messages.upsert", async (m) => {
                ...
                for (const message of messages) {
                    try {
                        // Ensure it's a status update with content and participant
                        if (!message || !message.key || message.key.remoteJid !== "status@broadcast") continue;
                        if (!message.message) continue;
                        if (!message.key.participant) continue;

                        // Throttle globally
                        const now = Date.now();
                        if (now - lastReactionTime < REACTION_INTERVAL_MS) {
                            // avoid flooding reactions
                            continue;
                        }

                        // Ensure bot user id available (only for logging/checks)
                        ...
                        console.log(`Successfully reacted to status by ${message.key.participant} with ${reactionEmoji}`);

                        // short delay to reduce burstiness
                        await (0, baileys_1.delay)(2000);
                    }
                    catch (err) {
                        console.error("Status reaction failed:", err && err.message ? err.message : err);
                        // swallow the error
                    }
                }
            });
